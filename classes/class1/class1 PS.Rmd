...
```{r}
# 1) define regression parameters
intercept2 <- 30
slope2 <- 10
residual_sd2 <- 10
...
y2_values <- intercept2 + slope2 * x2

# 4) add the residual standard deviation aka error
set.seed(2)
error2 <- rnorm(length(x), mean = 0, sd = residual_sd2)
y2_witherror <- y2_values + error2

# 5) create df with x & y
df2 <- tibble(x = x, y = y2_witherror)

# 6) plot 
plot(df2)

# 7) add regression model
RM2 <- stan_glm(y2_witherror ~ x, data = df2)
print(RM2)
abline(RM2)
...
```{r}
# Setting wd & loading in data
setwd("/Users/paulasamide/Desktop/AU/semester 2/methods/resources/classes")
allnames <- read.csv("data/Names/data/allnames_clean.csv", header = TRUE)

# Investigating data
colnames(allnames)
unique(allnames$sex)

# Filtering out the girlies
female_names <- filter(allnames, sex == 'F')

# Get the last letter in their names
